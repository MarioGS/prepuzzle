#' Converting ms SDTM into covariates input for the puzzle function
#'
...
#' @examples
#'
#'  ms = as.data.frame(prepuzzle_ms(df = df_dv1))

prepuzzle_ms = function(directory=NULL,
                        xpt=FALSE,
                        sas7bdat=FALSE,
                        csv=FALSE,
                        df,
  ...
  required = c("usubjid", "msdtc", "mstest", "msorres")
  ...
  if (required %!in% names(df)) {
    stop("You need to provide at least the following items: usubjid, msdtc, mstest and msorres")
  }
  
  df$ID = df$usubjid
  df$DATETIME = df$msdtc
  df$VARIABLE = df$mstest
  df$VALUE = df$msorres
