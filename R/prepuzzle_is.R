#' Converting is SDTM into covariates input for the puzzle function
#'
...
#' @examples
#'
#'  is = as.data.frame(prepuzzle_is(df = df_dv1))
...
prepuzzle_is = function(directory=NULL,
  ...
  required = c("usubjid", "isdtc", "istest", "isorres")
  ...
  if (required %!in% names(df)) {
    stop("You need to provide at least the following items: usubjid, isdtc, istest and isorres")
  }
  
  df$ID = df$usubjid
  df$DATETIME = df$isdtc
  df$VARIABLE = df$istest
  df$VALUE = df$isorres
