#' Converting lb SDTM into covariates input for the puzzle function
#'
...
#' @examples
#'
#'  lb = as.data.frame(prepuzzle_lb(df = df_dv1))

prepuzzle_lb = function(directory=NULL,
                        xpt=FALSE,
                        sas7bdat=FALSE,
                        csv=FALSE,
                        df,
  ...
  df$ID = df$usubjid
  df$DATETIME = df$lbdtc
  df$VARIABLE = df$lbtest
  df$VALUE = df$lborres
