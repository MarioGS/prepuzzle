#' Converting eg SDTM into covariates input for the puzzle function
#'
...
#' @examples
#'
#'  eg = as.data.frame(prepuzzle_eg(df = df_eg1))

prepuzzle_eg = function(directory=NULL,
                        xpt=FALSE,
                        sas7bdat=FALSE,
                        csv=FALSE,
                        df,
  ...
  required = c("usubjid", "egdtc", "egtestcd", "egorres")
  ...
  if (required %!in% names(df)) {
    stop("You need to provide at least the following items: usubjid, egdtc, egtestcd and egorres")
  }
  
  df$ID = df$usubjid
  df$DATETIME = df$egdtc 
  df$VARIABLE = df$egtestcd
  df$VALUE = df$egorres
