#' Converting dv SDTM into covariates input for the puzzle function
#'
...
#' @examples
#'
#'  dv = as.data.frame(prepuzzle_dv(df = df_dv1))

prepuzzle_dv = function(directory=NULL,
                        xpt=FALSE,
                        sas7bdat=FALSE,
                        csv=FALSE,
                        df,
  ...
  required = c("usubjid", "dvstdtc", "dvdecod", "epoch")
  ...
  if (required %!in% names(df)) {
    stop("You need to provide at least the following items: usubjid, dvstdtc, dvdecod and epoch")
  }
  
  df$ID = df$usubjid
  df$DATETIME = df$dvstdtc
  df$VARIABLE = df$dvdecod
  df$VALUE = df$epoch
